...
namespace kisyshot::compiler {
    ...
    void Sema::traverseExpression(
        ...
        case ast::syntax::SyntaxType::BinaryExpression: {
            auto&& e =
                std::dynamic_pointer_cast<ast::syntax::BinaryExpression>(expr);
        ...
        case ast::syntax::SyntaxType::UnaryExpression: {
            auto&& e =
                std::dynamic_pointer_cast<ast::syntax::UnaryExpression>(expr);
            traverseExpression(e->right);
            break;
        }
        case ast::syntax::SyntaxType::ParenthesesExpression: {
            auto&& e =
                std::dynamic_pointer_cast<ast::syntax::ParenthesesExpression>(
                    expr);
        ...
        case ast::syntax::SyntaxType::IndexExpression: {
            auto&& e =
                std::dynamic_pointer_cast<ast::syntax::IndexExpression>(expr);
            ...
            if (_arrLayer != 1) {
                ...
                e->layer = def->dimension[def->dimensionDef.size() - _arrLayer + 1];
            }
        ...
        case ast::syntax::SyntaxType::ArrayInitializeExpression: {
            for (auto&& e : std::dynamic_pointer_cast<
        ...
        case ast::syntax::SyntaxType::CallExpression: {
            auto&& e =
                std::dynamic_pointer_cast<ast::syntax::CallExpression>(expr);
    ...
    std::pair<int, bool> Sema::checkCompileTimeConstExpr(
        ...
        case ast::syntax::SyntaxType::UnaryExpression:{
            auto unary = std::dynamic_pointer_cast<
                              ast::syntax::UnaryExpression>(expr);
            int right;
            bool ok;
        ...
        case ast::syntax::SyntaxType::BinaryExpression:{
            auto binary = std::dynamic_pointer_cast<
                              ast::syntax::BinaryExpression>(expr);
            int left, right;
            bool ok;
        ...
        case ast::syntax::SyntaxType::IdentifierExpression:{
            auto id = std::dynamic_pointer_cast<ast::syntax::IdentifierExpression>(expr);
            ...
            case ast::syntax::SyntaxType::VariableDeclaration: {
                for (auto& def :
                    ...
                    if (!def->dimensionDef.empty()) {
                        if (def->initialValue != nullptr) {
                            flattenArray(def, std::dynamic_pointer_cast<ast::syntax::ArrayInitializeExpression>(def->initialValue), s);
                        }
    ...
    void Sema::flattenArray(const std::shared_ptr<ast::syntax::VarDefinition>& def,const std::shared_ptr<ast::syntax::ArrayInitializeExpression> &init, size_t target, size_t dim){
        ...
        if (init != nullptr){
            for (auto &i : init->array) {
                if(i->getType() == ast::syntax::SyntaxType::ArrayInitializeExpression){
                    ...
                    flattenArray(def, std::dynamic_pointer_cast<ast::syntax::ArrayInitializeExpression>(i), 
                        innerTarget, dim + 1);
                } else {
        ...
        while (def->srcArray.size() < current + target) {
            auto e =
                    std::make_shared<ast::syntax::NumericLiteralExpression>();
            e->rawCode = "0";
